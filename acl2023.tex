...
justify the actions of the agent by aligning them to natural language guidelines $d$.
...
\end{equation}
This is motivated by the VAE objective,
which optimizes $\log p(x) - KL[q(d\mid x) || p(d\mid x)]$,
a lower bound based on the reverse KL.
The forward KL has nice properties, and has been found to be useful
in variational inference \cite{fkl,rws}.
...
by becoming overconfident about an incorrect $d$.
...
\end{equation}
the forward KL divergence between 
