...
This if-then structure is a defining trait for guidelines,
as the guidelines specify a set of rules that the agent should proceed
through one after the other \cite{abcd}.
...
align each turn in $x$ to a sentence $z$ in natural language guidelines $d$.
...
unsupervised accuracy, in particular the document choice model degenerated to a uniform distribution.
...
We target the following objective:
\begin{equation}
\label{eqn:fkl}
\log\sum_d p(x,d) - KL[p(d\mid x) || q(d \mid x)].
\end{equation}
This differs from the VAE objective,
$\log p(x) - KL[q(d\mid x) || p(d\mid x)]$,
in that we use the forward KL.
...
In particular, the reverse KL is mode-seeking, meaning $q(d|x)$ may have smaller
tails than $p(d|x)$.
These smaller tails potentially lead to increased bias in gradient estimation.

We make two approximations to equation \ref{eqn:fkl} for computational efficiency.
Rather than relying on importance sampling, we rely on a top-$k$ approximation,
and use this top-k approximation for both
approximating the evidence $p(x)$ and a self-normalized
approximation of the posterior $p(z|x)$ and $q(z|x)$:
\begin{align*}
...
The topk operation is not differentiable.
...
Second, the VAE objective optimizes reverse KL, which has shown to have
pathologies not present in the reverse KL \cite{fkl}.

\paragraph{Relation to wake sleep} The wake-phase aims to find
\begin{equation}
\argmax_p \Es{x\sim D}{\Es{z\sim q(z\mid x)}\log p(x,z)/q(z\mid x)},
\end{equation}
and the sleep phase finds
\begin{equation}
\argmin_q \Es{x,z\sim p(x,z)}{\log \frac{p(x\mid z)}{q(z\mid x)}},
\end{equation}
...
and do not require samples from the generative model.\footnote{
    We could also potentialy optimize the evidence minus the symmetrized KL,
    leading to 
}
...
\section{Method 2: Sentence alignments}
Assert no multi-hop.
