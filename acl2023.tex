...
\section{Report and questions 2/27}
...
\item Approximate marginalization with $Z^*$: $\log\sum_{z\in Z*} p(x\mid z) p(z)$
    ...
    \item uniform $p(z)$
    \item BART $p(x \mid z)$
    \item Inference via Bayes' rule: $\argmax_z p(z\mid x_{1:t})$ where $t$ is the index
...
This is surprising, since the training setup (all Z) is closer to the testing setup (all Z), as Z* is a strict subset of Z
...
Are there negatives that were excluded by Z*, that end up making performance worse?
Hyperparam sweep for full marg.
Speed up full marg with an inference network q(z|x) in the VAE setting.
...
\item I would like your conclusions to be a little bit more clear about things like
    ...
    \item General resaonableness: Full marg is reasonable as long as it fits within memory constraints.
        It is reasonable for this particular dataset, but may not be for the other datasets.
    \end{itemize}
...
\item I don't really get this line "This is surprising, since the training setup (all Z) is closer to the testing setup (all Z), as Z* is a strict subset of Z". This doesn't seem surprising to me?
    ...
    \item When would approx marg w/ Z* do better?
        Hypothesis 1: If Z* contains hard negatives z with p(x | z*) > p(x | z) but not negatives p(x | z*) < p(x | z), so that the model doesnt learn to prefer those hard negatives over the true z*
        When would approx marg w/ Z* do worse?
        Hypothesis 2: If Z* misses some hard negatives with p(x | z*) > p(x | z)
        This means the result of approx marg w/ Z* is currently not informative in terms of comparison to full marginalization, without further error analysis that verifies Hypothesis 1.
    \end{itemize}
